...
drop table if exists clean.most_recent_fib4;

create table clean.most_recent_fib4 as (
    select 
        plt.entity_id, 
        plt.as_of_date,
        ...
        case when plt.result_num!=0 and alt.result_num!=0 then 
        round(cast((extract(year from age(plt.specimen_date::date, d.birth_date::date)) * AST.result_num) / 
        ((plt.result_num) * SQRT(ALT.result_num)) as numeric),2) AS fib4
    from clean.most_recent_labs plt left join clean.most_recent_labs ast 
...
create index on clean.most_recent_fib4(entity_id);
create index on clean.most_recent_fib4(as_of_date);
create index on clean.most_recent_fib4(days_since_plt);
create index on clean.most_recent_fib4(days_since_alt);
create index on clean.most_recent_fib4(days_since_ast);
create index on clean.most_recent_fib4(gap_between_tests);